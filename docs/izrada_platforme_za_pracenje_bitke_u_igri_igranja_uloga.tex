...
\sazetak{U ovom radu prikazan je razvoj i funkcionalnosti platforme za plaćenje bitke u igri igranja uloga s glavnim fokusom na
    implementaciju pravila igre Dungeons \& Dragons.
    Teorijski dio prolazi kroz povijest razvoja igri igranja uloga, od korijena u obuci časnika u zapovijadanju i planiranju bitke,
    do današnjih igara. Nadalje opisuje sustav Dungeons \& Dragons kao jedan od najpoznatijih sustava za igranje igra uloga.
    ...
    dijelova aplikacije te uz primjere koda prikazani neki zanimljivi dijelovi implementacije i arhitekture aplikacije.
}
...
Igre igranja uloga (engl. \textit{Role playing games, RPG}),
su doživjele značajan porast u popularnosti u posljednjem desetljeću, kako u klasičnim
...
Prvi dio rada posvećen je teorijskoj obradi igara igranja uloga i upoznavanju s sustavom Dungeons \& Dragons.
Sustav je dosta opsežan te se u ovom uglavnom spominju i objašnjavaju 
...
postavljanja scena i bitke prikazan na kraju.
...
|-- 1. Pozadina s poljima
 ...
    |-- 2. Prijašnja lokacija tokena
    |   |
    |   |-- Sličica tokena
    |   |-- Polja preko kojih je tokena prenesen
    |
    |-- 3. Token kontejner
        |
        |-- Sličica
...
iz razloga što je klijent u tom trenutku u potpunosti pridruženi igri i ima sve potrebne podatke za 
...
ili \texttt{action} događaja u entitet potrebno je registrirati vrstu entiteta u globalnoj instanci klase 
...
entitetu, kad klijent primi podatke od poslužitelja, može ih dobiti u bilo kakvom redoslijedu te se određivanjem
...
Ove funkcije se pozivaju kad klijent primi podatke od entitetima od poslužitelja.
...
su postavljanje najnoviji dobiveni podaci.
Ako entitet ne postoji i nije dodan u \texttt{EntityRegistry}-u tada se pokušavaju dohvatiti
entiteti \texttt{Scene} i \texttt{RpgTokenData}, o ovim entitetima ovisi entitet \texttt{RpgToken}
...
rekonstruirati cijelo stanje igre kod inicijalnog spajanja klijenta.
...
Definirani osnovni entiteti su dovoljni za proširenje i implementaciju sustava za pračenje bitki te se koriste
...
\begin{itemize}
    \item \textbf{RpgScene}: proširenje scene, sadrži dodatne funkcionalnosti za scene, na primjer,
        ...
        za \texttt{Token} entitete (likovi) na sceni, provjeru i prikaz udaljenosti koju koju se neki lik može
    ...
    \item \textbf{RpgToken}: proširenje \texttt{Token} entiteta. Interno koristi \texttt{RpgTokenData} entitet za dohvaćanje podataka
        ...
        D\&D pravilima.
    \item \textbf{TurnOrder}: entitet korišten za pračenje poteza i odgovoran za cijeli tok bitke. 
        Sprema podatke o redoslijedu poteza, koji lik je trenutno na potezu i koju su uključeni u borbu,
...
\begin{figure}[H]
    ...
    \caption{Dijagram slučajeva korištenja aplikacije}
...
S desne strane se nalazi se meni za upravljanje podacima o tokenima, upravljane bitkom
...
izborniku za odabir scena. Scena koja je postavljena kao prva scena će uvijek biti prikazana
...
Odabirom sloja omogućava se uređivanje tokena na tom sloj, ovo omogućava izolaciju između
tokena na različitim slojevima, tokene koji se nalaza ne drugim slojevima nije moguće uređivati.
...
\begin{figure}[H]
    ...
    \caption{Učitavanje pozadinske slike scene}
...
U ovom prozoru se također može postaviti veličina lika, koja će odgovarati veličini lika na scenu.
...
\begin{figure}[H]
    ...
    \caption{Isječak postavka tokena}
...
\begin{figure}[H]
    ...
    \caption{Odabir tokena za bitku}
...
Nakon ponovnog dolaska na potez brzina će biti resitirana na inicijalnu vrijednost
brzine koju taj token ima.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.5\textwidth]{slike/scene/12-encounter-in-combat.png}
    \caption{Početak borbe}
...
Tokom odabira pozicije, ispod tokena je prikazan indikator udaljenosti koju će token preći, jednak broj
prikazan u indikatoru će na kraju pomaka biti oduzet od tokenove trenutne brzine.

\begin{figure}[H]
    \centering
    \includegraphics[width=0.8\textwidth]{slike/scene/13-moving-in-battle.png}
    \caption{Pomicanje tokom borbe}
...
Na slici je igrač odabra napasti neprijatelja koristeći napad \texttt{Handaxe}.
...
Ovo je vidljivo na slici, tokeni pored su označeni dok token izvan dometa nema 
crveni obrub.
...
Drugu prednost ovakvog pristupa je vezana uz napade koji imaju veći doseg.
...
\begin{figure}[H]
    ...
    \caption{Bacanje kockice za napad}
...
Razdjela platforme na poslužitelj i klijent infrastrukturu je omogućilo
fleksibilnost kod implementacije D\&D sustava na klijentu,
pošto poslužitelj zahtjeva minimalno strukturirane podatke te je na sličan način moguće
...
Također, korisničko sučelje, iako funkcionalno, zahtjeva daljnju doradu u smislu
intuitivnosti i preglednosi za korisnike.
...
koncepta zidova i vertikalnih razina te podrške za zvučne efekte bi značajno
